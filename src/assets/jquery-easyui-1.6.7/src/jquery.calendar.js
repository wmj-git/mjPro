...
(function($){
	...
	function setSize(target, param){
		...
		if (t.find('.calendar-em_menu').is(':visible')){
	...
	function init(target){
		$(target).addClass('calendar').html(
				...
				'<div class="calendar-body">' +
					'<div class="calendar-em_menu">' +
						'<div class="calendar-em_menu-year-inner">' +
							'<span class="calendar-nav calendar-em_menu-prev"></span>' +
							'<span><input class="calendar-em_menu-year" type="text"></input></span>' +
							'<span class="calendar-nav calendar-em_menu-next"></span>' +
						'</div>' +
						'<div class="calendar-em_menu-month-inner">' +
						'</div>' +
					'</div>' +
				'</div>'
		);
	...
	function bindEvents(target){
		var opts = $.data(target, 'calendar').options;
		var menu = $(target).find('.calendar-em_menu');
		menu.find('.calendar-em_menu-year').unbind('.calendar').bind('keypress.calendar', function(e){
			if (e.keyCode == 13){
				setDate(true);
			}
		});
		$(target).unbind('.calendar').bind('mouseover.calendar', function(e){
			...
			var t = toTarget(e.target);
			if (t.hasClass('calendar-em_menu-next') || t.hasClass('calendar-nextyear')){
				showYear(1);
			} else if (t.hasClass('calendar-em_menu-prev') || t.hasClass('calendar-prevyear')){
				showYear(-1);
			} else if (t.hasClass('calendar-em_menu-month')){
		...
		function setDate(hideMenu){
			var menu = $(target).find('.calendar-em_menu');
			var year = menu.find('.calendar-em_menu-year').val();
		...
		function showYear(delta){
			opts.year += delta;
			show(target);
			menu.find('.calendar-em_menu-year').val(opts.year);
		}
	...
	/**
	 * show the select em_menu that can change year or month, if the em_menu is not be created then create it.
	 */
	function showSelectMenus(target){
		var opts = $.data(target, 'calendar').options;
		$(target).find('.calendar-em_menu').show();
		
		if ($(target).find('.calendar-em_menu-month-inner').is(':empty')){
			$(target).find('.calendar-em_menu-month-inner').empty();
			var t = $('<table class="calendar-mtable"></table>').appendTo($(target).find('.calendar-em_menu-month-inner'));
			var idx = 0;
			for(var i=0; i<3; i++){
				var tr = $('<tr></tr>').appendTo(t);
				for(var j=0; j<4; j++){
					$('<td class="calendar-nav calendar-em_menu-month"></td>').html(opts.months[idx++]).attr('abbr',idx).appendTo(tr);
				}
			}
		}
		
		var body = $(target).find('.calendar-body');
		var sele = $(target).find('.calendar-em_menu');
		var seleYear = sele.find('.calendar-em_menu-year-inner');
		var seleMonth = sele.find('.calendar-em_menu-month-inner');
	...
	$.fn.calendar = function(options, param){
		...
		return this.each(function(){
			...
			bindEvents(this);
			show(this);
			$(this).find('div.calendar-em_menu').hide();	// hide the calendar em_menu
		});
	};
